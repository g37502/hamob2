...
from django.db.models.fields.related import ForeignKey,ManyToManyField,OneToOneField,ManyToManyRel
...
class Row(object):
    def __init__(self,data_list,option,query_dict):
        ...
        # logger.debug(data_list,option,query_dict)
...
class Option(object):
    ...
    def get_queryset(self, _field, model_class,query_dict):
        ...
        elif isinstance(_field,ManyToManyRel):
            s = _field.related_model.objects.all()
            # print(s,type(s))
            row = Row(s,self,query_dict)
        else:
            if self.is_choice:
...
class ChangeList(object):
    ...
    def __init__(self, config, queryset, q, search_list, page=None):
        self.q = q
...
class StarkConfig(object):
    ...
    def get_del_url_name(self):
        ...
        return name
    @property
    def get_edit_del_url_name(self):
        app_label = self.model_class._meta.app_label
        model_name = self.model_class._meta.model_name
        if self.prev:
            name = '%s_%s_%s_edit_del' % (app_label,model_name,self.prev)
        else:
            name = '%s_%s_del' % (app_label,model_name)
        return name
